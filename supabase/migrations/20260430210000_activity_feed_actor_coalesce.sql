-- Update activity_feed view to prefer the explicit header-driven
-- actor_member_id (from audit.row_history.actor_member_id, populated by
-- the audit trigger reading the X-Audit-Actor request header) before
-- falling back to the auth.uid() → member_emails resolution.
--
-- Run order: must come after 20260430200000_audit_actor_member_id.sql
-- which adds the actor_member_id column to audit.row_history.
...
AS
  ...
  SELECT
    ...
    -- Prefer the explicit header-driven actor_member_id (admin server actions
    -- attach X-Audit-Actor); fall back to auth.uid() resolution (portal
    -- self-edits using the session client).
    COALESCE(
      ar.actor_member_id,
      (
        SELECT me.member_id
