...
- **Replaces Streak mail merge.** Bottom section of `/admin/emails`. Personal one-to-one-looking email: `Hi <first_name>,` + rich-text body + Eric's LIVE Gmail signature (fetched from Gmail settings at send time). No Thunderview template, no unsubscribe footer. Sent from eric@marcoullier.com via Gmail API — lands in his Sent folder, replies thread in his inbox.
