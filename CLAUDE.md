...
- **People feed filter**: `actor_id IS NOT NULL` AND source ≠ `email_events` AND event_type NOT LIKE `cron.%`/`webhook.%`/`error.%` AND event_type ≠ `email.transactional_sent`.
- **System feed inclusion list** (operational failures only, NOT a firehose): `error.caught`, `email.bounced`, `email.complained`, `email.failed`. Defined as `SYSTEM_FEED_INCLUDED_TYPES` in `src/lib/activity-feed.ts`. Every other event still lands in the underlying tables for ad-hoc SQL but does not appear in the System tab. The dropdown on the System tab returns the four inclusion-list types directly (not driven by what's in the data), so an admin can pre-filter even before any failure has fired.
- **Cron heartbeat convention**: every cron fire emits exactly one `cron.<name>` row. Success-with-action runs put summary counts in metadata; success-no-op runs add `metadata.outcome = 'no_op'`; failures emit `error.caught` instead. Heartbeats are NOT in the System feed inclusion list — they exist for ad-hoc "did the cron fire today" queries against `system_events`.
- **Helpers** (in `src/lib/`):
