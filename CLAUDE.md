...
- **Profile pictures** (Sprint 6): Supabase Storage bucket `profile-pics` (public-read, authenticated-write, RLS: members can only upload at their own member_id path). Column `members.profile_pic_url` TEXT NULL stores full public URL with `?v={timestamp}` cache-bust. Upload on `/portal/profile`: server-side processing via `sharp` — resize to 400×400, center-crop, convert to WebP, strip EXIF. Max 5MB, accepts JPEG/PNG/WebP/HEIC. Stored at `profile-pics/{member_id}.webp` (upsert overwrites). Reusable `<MemberAvatar>` component (`src/components/member-avatar.tsx`): shows pic if set, initials circle if not. Sizes: sm (28px), md (40px), lg (120px). Displayed in 7 locations: portal profile upload, portal member profile page, community table rows, recap cards, top-nav dropdown trigger, admin member detail heading, admin members table rows, admin dinner detail ticket rows.
