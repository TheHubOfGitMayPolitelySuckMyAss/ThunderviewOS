...
- **Stage precedence (9 stages, top-down, first match wins):** `Team` (is_team) → `Opted Out` (!marketing_opted_in OR kicked_out) → `Bounced` (≥1 email AND every email bounced; vacuous zero-email case excluded) → `Has Ticket` (purchased/fulfilled ticket for a dinner today-or-later MT) → `Not This One` (active exclusion for a today-or-later dinner) → `Investors` (stagetype `Investor` AND NOT `Active CEO (Bootstrapping or VC-Backed)` — investor-who-is-also-CEO segments as CEO) → `Attended` → `Approved` → `Applied`. Pure function in `compute-stage.ts`. Stage names must match Streak verbatim — bootstrap throws on missing stages, doesn't auto-create them.
...
- **Inbound webhook auth via `?secret=` query param** (`STREAK_WEBHOOK_SECRET`) — Streak's automation builder lacks HMAC and custom headers. Routes (`/api/webhooks/streak/{opted-out,not-this-one,bounced}`) return 200 on internal errors so Streak doesn't retry indefinitely; only secret-check failures return 401. `bounced` runs the same hard-bounce cascade as the Resend webhook (shared `applyHardBounce` helper in `src/lib/email-bounce.ts`), keyed off the box's primary email — used when a bounce returns as unstructured text Resend can't classify.
...
- **Hard-bounce cascade emits explicit system events** so the System feed shows what we DID, not just the inbound notification. Two entry points (Resend Permanent bounce, Streak Bounced webhook) converge on `applyHardBounce` in `src/lib/email-bounce.ts`, which fires in side-effect order: `email.status_set_bounced` (flipped `email_status='bounced'`), then exactly one of `email.primary_promoted` / `email.no_secondary_available` / neither-if-secondary-bounced, then `streak.bounce_synced` (metadata.outcome ∈ `primary_rotated | no_secondary_unreachable | secondary_retired`). All in `SYSTEM_FEED_INCLUDED_TYPES`. Promotion failure still lands as `error.caught` with `cause='promote_secondary_failed'`. `actor_label` distinguishes entry points (`webhook:resend` vs `webhook:streak`).
...
- **Hard vs soft bounces distinguished.** `bounce.type='Permanent'` flips `email_status='bounced'`, runs secondary promotion, pushes Streak immediately. Transient / Undetermined → row persists in `email_events`. **Second** soft bounce on the same address (when `email_status` is still `active`) escalates to the same cascade and emits `email.soft_bounce_escalated` first for traceability. Reason: AWS SES (via Resend) classifies DNS-resolution failures as Transient even when the receiving domain is permanently gone, so a single soft bounce under-signals reality.
