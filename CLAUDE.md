...
- **All three marketing-email draft editors are protected by `useUnsavedChangesGuard`** (`src/lib/use-unsaved-changes-guard.ts`). Two guarantees as long as the user has unsent edits: (a) a debounced auto-save fires 1.5s after the last keystroke and syncs all text fields to the DB via `saveDraft`; (b) `beforeunload` registers so refresh/tab-close/external-nav prompts the browser's "Leave site?" dialog. Auto-save resets `hasEdited` on success — beforeunload stops warning, Save Draft button greys out. `markEdited()` in each editor increments `editVersion`, which is the only thing that resets the debounce. The hook does NOT intercept Next.js client-side `<Link>` navigations (App Router has no API for that yet) — auto-save covers that gap by pushing latest text to the DB before most navigations.
