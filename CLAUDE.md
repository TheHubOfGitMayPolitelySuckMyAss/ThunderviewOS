...
- **Seat cap: 45/dinner, derived not stored.** `SEAT_CAP` + `getSeatsSold(ByDinner)` in `src/lib/seat-cap.ts` — SUM(`tickets.quantity`) over purchased/fulfilled (guest ticket = 2 seats). Enforced in `purchaseTicket` (`seatsSold + quantity > SEAT_CAP` → redirect); portal pages thread `seatsLeft` into `TicketPurchase` for sold-out UI (guest button hidden at 1 seat left). Refund/credit reopens sales automatically. Accepted race: Checkout sessions in flight at cap can overshoot — webhook doesn't decline. Helper THROWS on DB error (failed count must not read as 0 seats sold).
