...
- **Seat cap: per-dinner `dinners.seat_cap` (NOT NULL DEFAULT 45, CHECK > 0); sold-out derived, not stored.** Seats sold = SUM(`tickets.quantity`) over purchased/fulfilled (guest ticket = 2 seats) via `getSeatsSold(ByDinner)` in `src/lib/seat-cap.ts`. Enforced in `purchaseTicket` (`seatsSold + quantity > dinner.seat_cap` → redirect); portal pages thread `seatsLeft` into `TicketPurchase` for sold-out UI (guest button hidden at 1 seat left). December dinners hold 80 — `seatCapForMonth` in seat-cap.ts, applied by the generate-dinner cron at insert; existing December rows backfilled by migration. Editable on `/admin/dinners/[id]` (Seat Cap tile → `updateDinnerSeatCap`). Refund/credit reopens sales automatically. Accepted race: Checkout sessions in flight at cap can overshoot — webhook doesn't decline. Count helper THROWS on DB error (failed count must not read as 0 seats sold).
...
The segmentation model. `src/lib/member-stage.ts` — pure function `computeStageForMember`, every member in exactly ONE bucket, first match wins. Consumed by the mail-merge audience engine.

- **Precedence (9 stages, top-down):** `team` (is_team, wins over everything) → `opted_out` (!marketing_opted_in OR kicked_out) → `bounced` (≥1 email AND every email bounced; vacuous zero-email case excluded) → `has_ticket` (purchased/fulfilled ticket for a dinner today-or-later MT) → `not_this_one` (active exclusion for a today-or-later dinner) → `investors` (stagetype `Investor` AND NOT `Active CEO (Bootstrapping or VC-Backed)` — investor-who-is-also-CEO segments as CEO) → `attended` → `approved`. (`applied` = pending applications, a different table.)
- **The Streak integration that birthed this ladder was ripped out 2026-07-13** (Eric cancelled Streak; mail merges replaced it). `members.streak_box_key` / `applications.streak_box_key` columns and historical `streak.*` system_events rows remain — dead data, don't build on them. `excluded_from_dinner_id` (the not_this_one input) is now written ONLY by the Gmail "TV Skip" label action (see Gmail label actions); post-dinner cron still clears stale values.
...
- **Hard-bounce cascade emits explicit system events** so the System feed shows what we DID, not just the inbound notification. `applyHardBounce` in `src/lib/email-bounce.ts` (sole entry point: Resend webhook) fires in side-effect order: `email.status_set_bounced` (flipped `email_status='bounced'`), then exactly one of `email.primary_promoted` / `email.no_secondary_available` / neither-if-secondary-bounced. All in `SYSTEM_FEED_INCLUDED_TYPES`. Promotion failure still lands as `error.caught` with `cause='promote_secondary_failed'`.
...
- **Seven transactional templates wired:** approval, re-application, rejection, fulfillment, morning-of, prompt-intro-ask, applied-didnt-convert. Editable at `/admin/emails/*`. From: `team@thunderviewceodinners.com`. Body uses `[member.fieldname]` placeholders.
- **Morning-of email has a hardcoded venue-tease block** (two headlines + two `email-images` photos: `morning-of-venue-{outside,inside}.jpg`) inserted in `sendMorningOfEmail` between the editable body and Tonight's Attendees. Not in the template — swap photos by overwriting the bucket files (URLs stable).
- **`prompt-intro-ask` is the only template stored as two rows** (`prompt-intro-ask-missing`, `prompt-intro-ask-stale`) because it has two conditional bodies, one per recipient situation. No template-engine syntax — `sendPromptIntroAskEmail(memberId, dinnerId, situation)` picks the slug at send time. Cron `/api/cron/prompt-intro-ask` fires daily 14:00 UTC, no-ops unless a dinner is scheduled exactly 2 days out (MT). For each ticketed member (`fulfillment_status IN ('purchased','fulfilled')`): missing = both intro AND ask blank → sends `*-missing`; stale = both filled AND `ask_updated_at <= last_dinner_attended` → sends `*-stale`; never-attended members with a filled ask are NOT sent (no "stale" reference point yet). Admin UI at `/admin/emails/prompt-intro-ask` shows both editors stacked.
...
- **Hard vs soft bounces distinguished.** `bounce.type='Permanent'` flips `email_status='bounced'` and runs secondary promotion. Transient / Undetermined → row persists in `email_events`. **Second** soft bounce on the same address (when `email_status` is still `active`) escalates to the same cascade and emits `email.soft_bounce_escalated` first for traceability. Reason: AWS SES (via Resend) classifies DNS-resolution failures as Transient even when the receiving domain is permanently gone, so a single soft bounce under-signals reality.
...
### Mail merges (Gmail, NOT Resend)

- **Replaces Streak mail merge.** Bottom section of `/admin/emails`. Personal one-to-one-looking email: rich-text body with `[member.first_name]` placeholder (same syntax as transactional templates; NO auto-greeting — Eric writes his own) + Eric's LIVE Gmail signature (fetched from Gmail settings at send time), exactly one blank line below the last body line (trailing empty paragraphs stripped; no `<br>`s at the body/sig junction — the final block's margin IS the blank line). No Thunderview template, no unsubscribe footer. Sent from eric@marcoullier.com via Gmail API — lands in his Sent folder, replies thread in his inbox.
- **Audience = the member-stage ladder** (see Member-stage ladder section). `src/lib/mail-merge/audience.ts` batch-fetches and runs `computeStageForMember` in memory. Selectable groups: `investors`/`attended`/`approved`; `team` always included. `opted_out`/`bounced`/`has_ticket`/`not_this_one` are structurally unsendable — no send path exists (Eric never merges ticket-holders: they're mid-transactional-flow; exclusions asked not to be contacted).
- **Async queue send.** `mail_merge_recipients` = frozen audience + send queue + per-bucket counts (one row per person, UNIQUE(merge,member), statuses pending/processing/sent/failed/skipped). Send action freezes rows FIRST, then flips draft→sending (drain ignores drafts, so a half-frozen queue can't finalize prematurely), drains the first ~4 min inline via `after()`; per-minute cron finishes and auto-resumes merges stranded by crash/deploy. Claim via `claim_mail_merge_recipient()` RPC — FOR UPDATE SKIP LOCKED, claim-ONE-at-a-time — so overlapping drains can never double-send. Pace ~1/sec (deliberate one-to-one look; Gmail Workspace daily cap 2,000). Fatal Gmail errors (401/403/429 = dead grant/quota) release the claim and abort the run; recipient-specific errors mark the row failed + `error.caught`. Stalled `processing` rows (>30 min) are auto-failed, NOT retried — the crash window includes "Gmail accepted, we died before recording it," and a retry could double-send.
- **`google_oauth_tokens`: single-row token store, RLS enabled with ZERO policies** (service-role only — raw tokens must never reach the authenticated role). Shares the DigiEric Google Cloud OAuth client (same `GOOGLE_CLIENT_ID`/`SECRET`) but holds its OWN grant — the apps never read each other's DBs. Connect/reconnect at `/api/auth/google` (admin-gated; scopes gmail.send + gmail.settings.basic only; `prompt=consent` so reconnecting always re-issues a refresh token). Thunderview's callback `https://thunderviewceodinners.com/api/auth/google/callback` must be whitelisted on that OAuth client in Google Cloud console.
- **Test-send gate same as one-off blast** (`test_sent_after_last_edit` reset on save; test goes to the acting admin via the full Gmail pipeline). Testing mode restricts send-to-all to the team bucket.
### Gmail label actions (TV Bounce / TV Skip / TV Opt Out)

- **Eric's in-email ops surface.** He labels a message in his own Gmail; the per-minute `gmail-label-actions` cron acts on it and replies in the same thread with what it did (or why it couldn't). No admin UI required. Labels: `TV Bounce` (run `applyHardBounce` — covers Gmail mail-merge bounces, which have no Resend webhook), `TV Skip` (set `excluded_from_dinner_id` to the next upcoming dinner → `not_this_one` stage), `TV Opt Out` (flip `marketing_opted_in` false — same write as the unsubscribe link; feed refiner renders the audit row as `member.marketing_opted_out`). `TV Done` / `TV Error` mark the outcome; failed messages are NOT retried until Eric re-applies the trigger label. Labels are auto-created via API. Archiving a labeled message is fine — the cron queries by label, not inbox; the confirmation reply (addressed to Eric) does pull the thread back into the inbox.
- **Requires the `gmail.modify` scope** — added to `/api/auth/google` 2026-07-14. Grants issued before that need a reconnect; until then the cron returns `not_configured` silently and `/admin/emails` shows a "Reconnect Gmail" link (`GmailConnection.labelScopeOk`).
- **Target email extracted by Claude** (`src/lib/gmail/extract-target.ts`, claude-opus-4-8): bounce → the failed recipient parsed out of the mailer-daemon content; skip/optout → the member who can't attend / wants off the list (usually From, body wins on forwards). NONE/garbage → `TV Error` + explanatory reply, nothing changed.
- **One action per (thread, kind), not per message.** Gmail applies a conversation label to every message in the thread, so one label click on an N-message thread arrives as N labeled messages (this triple-emailed Eric 2026-07-16). The cron groups by (threadId, kind) and acts once: candidates tried non-Eric-From first (Eric's own replies extract to NONE), first success wins → one reply, all siblings → TV Done. All candidates fail → one error reply, all siblings → TV Error. A re-applied label on an already-handled thread goes silently to Done unless the thread gained a new processable message.
- **Audit attribution = Eric** via `createAdminClientForActor(ericMemberId)` (resolved by hardcoded email — he acted by labeling; there's no session). Dedupe on `gmail_message_id` via `system_events` rows of type `gmail_label.processed`, so a crash between acting and re-labeling can't double-apply. `member.excluded_from_dinner` is in `SYSTEM_FEED_INCLUDED_TYPES`; bounce-cascade events already were.
- **`GmailFatalError` (401/403/429) aborts the run**; cron-level throws log `error.caught` at most once/hour (per-minute cadence would flood system_events with an identical dead-grant error).

- **`requireAdminOrTeam()` in `src/lib/require-admin.ts`** — session→member admin/team check for API routes and server actions (the proxy only guards /admin PAGE paths). Mail-merge actions and the Google OAuth routes use it; older marketing-email actions predate it and only check "is a member."
...
- `/api/cron/post-dinner` — daily. If yesterday was a dinner, updates `last_dinner_attended` for fulfilled-ticket holders. Also clears stale `excluded_from_dinner_id` (expired NTO state; no writer since the Streak teardown). NTO clear errors logged, don't fail the cron.
...
- `/api/cron/mail-merge-drain` — every minute. Drains mail-merge send queues (see Mail merges). **Heartbeat deviation:** silent when no merge is `sending` — 1,440 fires/day would flood `system_events`; emits `cron.mail_merge_drain` only when it actually drained.
- `/api/cron/gmail-label-actions` — every minute. Processes TV Bounce / TV Skip labels in Eric's Gmail (see Gmail label actions). Same heartbeat deviation as mail-merge-drain: emits `cron.gmail_label_actions` only when it processed something; silent when idle or when the grant lacks `gmail.modify`.
- `/api/cron/coachingos-attendee-sync` — daily 12:00 UTC. No-op unless today MT is a dinner day. POSTs to CoachingOS at `COACHINGOS_WEBHOOK_URL` (bearer `COACHINGOS_WEBHOOK_SECRET`) the members with a fulfilled ticket for today's dinner where `last_dinner_attended IS NULL` (first-time attendees) **OR `coachingos_resend_requested = true`** (prior no-shows re-armed by DigiEric — see below). Payload per attendee: member id, name, email, company name/website, linkedin, intro, ask. On a confirmed 2xx, clears `coachingos_resend_requested` for everyone pushed (one-shot pulse). Best-effort: webhook failure → `error.caught`, cron still 200 (no retry storm), flag left armed for next run; CoachingOS dedups on its end. **This is the only Thunderview→CoachingOS data flow.**
  - **No-show re-send loop (the only CoachingOS→Thunderview flow).** A no-show still gets `last_dinner_attended` stamped by `post-dinner` (it keys off fulfilled tickets, not actual attendance), so they'd never re-surface in DigiEric. DigiEric's "Didn't come" button deletes the candidate there and POSTs `{ thunderview_member_id }` to `/api/webhooks/coachingos/no-show` (bearer `COACHINGOS_WEBHOOK_SECRET`, 200-on-error), which sets `members.coachingos_resend_requested = true`. Next dinner they hold a fulfilled ticket, the sync re-includes them and clears the flag. Re-armed by each "Didn't come"; "Dismiss" in DigiEric sends nothing here, so genuine attendees are never re-sent. `coachingos_resend_requested` is webhook-set / cron-cleared — not trigger-managed.
...
- `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET` — Google OAuth client shared with DigiEric, used for mail-merge Gmail sending. Production + local. Preview is NOT wired (preview callback URLs aren't whitelisted on the OAuth client).
...
- **Apply form has a zero-friction anti-spam gate (honeypot + signed timing token).** `submitApplication` (`src/app/apply/actions.ts`) drops bots BEFORE the member check / insert: (a) a hidden `company_url_confirm` honeypot field (invisible to humans, read via ref in `application-form.tsx`) — any non-empty value is spam; (b) an HMAC-signed token minted server-side at page render (`src/lib/form-token.ts`, reuses `UNSUBSCRIBE_SECRET`) — rejected if the signature fails, the form was submitted in <3s (`MIN_FILL_MS`), or the token is >12h old (`MAX_AGE_MS`). `apply/page.tsx` is `force-dynamic` so each visitor gets a fresh token. Spam is dropped **silently** — returns the normal `{ success: true }` so the bot lands on `/apply/thanks` and can't tell it was filtered (no row, no admin email). Each drop logs `application.spam_blocked` to `system_events` (ad-hoc query only — deliberately NOT in any activity-feed inclusion list). No CAPTCHA / no third-party account; Cloudflare Turnstile is the next lever if low-effort honeypot bots aren't the whole problem.
- **Public application submissions short-circuit when email matches an active member.** `src/app/apply/actions.ts` checks `member_emails` before insert. Active (non-kicked-out) match → returns `{ success: true, alreadyMember: true }`, no application row, no admin notification; form redirects to `/apply/already-member`. Kicked-out re-applications fall through to the pending flow and are flagged with a red "Removed-member re-application" pill in the admin queue.
- **Approve/Reject from the new-application notification email** via signed links to the public `/application-review` route. The email (`sendNewApplicationNotification`) carries Approve/Reject buttons + the full "Review Application" deep link. **The link is GET-safe — it only renders a confirmation page; the mutation runs from a POST server action (`submitApplicationReview`), so email/scanner prefetch can't fire it.** Token (`src/lib/application-action-token.ts`) is HMAC-signed over `applicationId:action` (an approve link can't be replayed as reject). The action re-verifies the token, re-checks `status='pending'` (no-op if already handled), resolves the admin actor by hardcoded email and attributes the audit row via `createAdminClientForActor(memberId)` (there's no session in this flow). Approve/reject **core logic lives in `src/lib/application-review.ts`** (`approveApplicationWith`/`rejectApplicationWith`, NOT a `"use server"` module) — shared verbatim with the admin-UI server actions, which are now thin wrappers. Reject still requires a reason (entered on the confirm page). Token secret: `APPLICATION_ACTION_SECRET` if set, else falls back to the provisioned `UNSUBSCRIBE_SECRET` — so no new Vercel env var is required.
...
- **Dashboard auto-refreshes on tab focus and shows "+N new" pills per accordion.** `dashboard-accordions.tsx` has two client hooks: `useAutoRefreshOnFocus` (visibilitychange → `router.refresh()`) and `useNewSinceLastView` (per-accordion seen-IDs cached in `localStorage` at key `tv:dashboard:seen:{key}`). On tab blur, current IDs become the new seen baseline; the next visible-event diffs against it. First visit ever: everything is treated as already-seen so the dashboard doesn't load with a sea of pills.
