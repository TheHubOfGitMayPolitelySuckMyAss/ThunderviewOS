...
### Gmail label actions (TV Bounce / TV Skip)

- **Eric's in-email ops surface.** He labels a message in his own Gmail; the per-minute `gmail-label-actions` cron acts on it and replies in the same thread with what it did (or why it couldn't). No admin UI required. Labels: `TV Bounce` (run `applyHardBounce` — covers Gmail mail-merge bounces, which have no Resend webhook), `TV Skip` (set `excluded_from_dinner_id` to the next upcoming dinner → `not_this_one` stage). `TV Done` / `TV Error` mark the outcome; failed messages are NOT retried until Eric re-applies the trigger label. Labels are auto-created via API.
...
- **Target email extracted by Claude** (`src/lib/gmail/extract-target.ts`, claude-opus-4-8): bounce → the failed recipient parsed out of the mailer-daemon content; skip → the member who can't attend (usually From, body wins on forwards). NONE/garbage → `TV Error` + explanatory reply, nothing changed.
