...
│   │   ├── dinner-details-block.tsx   # Server component: next dinner title/description/speakers block (shown when no ticket)
...
- **Portal home page** (`/portal`): single-column layout (`tv-container-narrow`, 640px). Two states: (1) member has an upcoming purchased/fulfilled ticket → confirmation banner ("You're confirmed for [date]") + Intro/Ask/Contact form in a Card; (2) no upcoming ticket → dinner details block (title/description/speakers for next upcoming dinner, pinned via `getTargetDinner()` logic) above inline ticket purchase widget in a Card. Dinner details block: shows "Topic and speakers announced soon." placeholder if all fields empty; otherwise shows whichever of title, description, speakers are present. Speakers render as `<MemberAvatar size="md">` + name + company, linked to LinkedIn (or website, or no link). Kicked-out and `has_community_access = false` members still shown as speakers. Block hidden in ticket-purchased state. Small "Edit your profile" link at bottom. No nav buttons — Community, Recap, Tickets, Profile are all accessible via top nav. Save button with toast on the Intro/Ask form.
