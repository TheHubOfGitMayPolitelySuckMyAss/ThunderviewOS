...
import AppliedDidntConvertEditor from "./template-editor";

export default async function AppliedDidntConvertTemplatePage() {
  const admin = createAdminClient("read-only");

  const { data: template } = await admin
    .from("email_templates")
    .select("*")
    .eq("slug", "applied-didnt-convert")
    .single();
  ...
  return (
    <div>
      ...
      <h2 className="tv-h2 !text-[36px] mb-6">
        Applied, Didn&rsquo;t Convert Email Template
      </h2>

      <AppliedDidntConvertEditor
        slug="applied-didnt-convert"
