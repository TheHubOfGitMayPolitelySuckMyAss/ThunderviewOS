...
- **One accent color:** `--clay-500 #B5835A`. Buttons, links, and key affordances. Hover darkens to `--clay-600`.
...
- **Semantic aliases only in app code.** Use `--border` and `--border-subtle` only. The `--line-*` scale values stay in `:root` as the underlying definitions but are not referenced in components. Same rule applies across the system: always prefer the semantic alias (`--bg`, `--fg1`, `--accent`) over the scale (`--cream-50`, `--ink-900`, `--clay-500`).
...
- **Token names:** Motion tokens are `--dur-fast` / `--dur-med` / `--dur-slow` and `--ease-out` / `--ease-in-out`. There is no `--tv-dur-*` prefix — any such reference is a bug.
...
- **Focus ring:** 2px `--clay-500` outline, 2px offset. Always visible, never removed.
...
### Layout rules
- **Max content width:** 1040px for marketing reading, 1280px for app screens, 1440px for admin tables.
- **Gutters:** 24px mobile, 48px desktop. Content feels roomy — this is a dinner, not a conference.
- **Sticky top nav** (see `ui_kits/portal` and `ui_kits/marketing`); no sticky footers.
...
These values are non-negotiable. If a new design needs to deviate, update the token — do not hand-tune a component. The goal is that any new page added in six months matches the existing pages without a designer re-specifying spacing.

- **Nav height:** All top navigation is exactly `var(--nav-height)` (64px) tall. Applies to public-nav, portal top-nav, and any future authenticated chrome. The logo is `var(--nav-logo-size)` (20px) Fraunces 500 with `-0.01em` tracking. Use the `.tv-nav` utility or compose the invariants directly.
- **Page gutters:** `var(--page-gutter-sm)` (24px) on mobile, `var(--page-gutter-lg)` (48px) on desktop. Apply via `.tv-page-gutter` or equivalent. Never hand-tune `px-6`, `px-7`, `px-10`.
...
- **Vertical rhythm:** `var(--section-gap)` (64px) between major sections. `var(--stack-gap)` (24px) inside a section. `var(--tight-gap)` (12px) for related items.
...
- **Forms:** Fields stack at `--form-row-gap` (16px). Label-to-input is `--label-input-gap` (8px). Button groups use `--button-group-gap` (12px).
- **Tables:** Rows are `--table-row-height` (52px). Headers are `--table-header-height` (44px). Cells are horizontally padded at `--table-cell-padding-x` (16px).
- **Modals:** sm 420px, md 560px, lg 720px. Use `--modal-width-*`.
- **Nav link spacing:** `--nav-link-gap` (24px) between nav links. `--nav-logo-to-links-gap` (48px) between logo and the link group.

---

## COMPONENTS

- **Buttons-as-links rule:** Anything that looks like a button must use the `Button` primitive. No anchor-styled-as-button. If a `<Link>` needs button visuals, wrap: `<Button asChild><Link href="…">Label</Link></Button>`. This prevents per-file reinvention of padding, focus ring, and hover behavior.
...
- **Stage-type badges:** small pill with `--bg-accent` background, `--ink-900` text, no icon. Let copy carry the meaning.
