/**
 * One-time bulk contact-attach for every existing Streak box.
 *
 * Prompt C populated 636 boxes with custom-column data, but Streak's mail-merge
 * UI pulls from the Contacts section — not custom columns. Boxes shipped from
 * Prompt C have empty Contacts. Prompt D wires the contact step into the push
 * primitives going forward; this script reaches back and attaches contacts
 * to every box that already exists.
 *
 *   npx tsx --env-file=.env.local tmp/streak-contacts-bulk.ts
 *
 * Idempotent. createContact uses getIfExisting=true (server-side dedup) and
 * addContactToBox replaces the box's contacts array — re-running produces the
 * same Streak state. No DB writes from this loop; we just trigger pushMember /
 * pushApplication which already do the work.
 *
 * Single-flight: tmp/streak-contacts-bulk.lock prevents concurrent runs.
 */
...
const LOCK_PATH = resolve(process.cwd(), "tmp/streak-contacts-bulk.lock");
// Defensive ceiling for SELECT pagination — prod members are ~636 today.
...
function acquireLock(): void {
  if (existsSync(LOCK_PATH)) {
    console.error(
      `Bulk contact run already in progress (or previous run crashed). Remove ${LOCK_PATH} to proceed.`
    );
    process.exit(1);
  }
...
async function fetchAppIds(sb: SupabaseClient): Promise<string[]> {
  const res = await sb
    .from("applications")
    .select("id")
    .not("streak_box_key", "is", null)
...
async function main() {
  ...
  console.log("=== Streak contacts bulk attach ===\n");

  resetStreakCache();
  console.log("[preflight] resolving Streak pipeline + team + stages + fields…");
  ...
  console.log("\n[preflight] counting boxes already in Streak…");
  const appIds = await fetchAppIds(sb);
  const memberIds = await fetchMemberIds(sb);
  console.log(`  applications with box:  ${appIds.length}`);
  console.log(`  members with box:       ${memberIds.length}`);
  console.log(`  total:                  ${appIds.length + memberIds.length}`);

  if (appIds.length === 0 && memberIds.length === 0) {
    console.log("\nNo boxes to update. Exiting.");
    return;
  }
  ...
  console.log("\n=== Summary ===");
  console.log(
    `Applications:  ${appIds.length} total, ${phase1.ok} succeeded, ${phase1.failed.length} failed`
  );
  console.log(
...
main().catch((err) => {
  console.error("Bulk run threw:", err);
  process.exitCode = 1;
});
